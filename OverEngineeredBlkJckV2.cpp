 ...
 * Author: Saim Ahmed
 * Date 6/6/25
 * Description: Play classic casino blackjack with yourself or with friends against an AI powered DealerTree!
 *              Place bets to win big (or lose) and place yourself on a communal leaderboard!
 *              Obtain a hand closest to 21 without going over, but don't let the DealerTree outscore you, or its bust!
...
//Average table size for blackjack, not including DealerTree.
...
public:
    ...
    */
    ...
    /*
        ...
        isHitting() isn't native to player class, as the DealerTree will have some logic
...
class DealerTree : public Player {  //Add Tree
private:
    Deck m_deck;
    struct DecisionNode {
        // Function WRAPPER that allows the called function to be referenced in constructor.
        // This is unironically the coolest thing I've come across while searching for things to use for the project.
        function<bool(int)> condition;
        // The action to take if the condition is met (true)
        bool result;
        // Child nodes for more complex decisions
        unique_ptr<DecisionNode> true_branch;
        unique_ptr<DecisionNode> false_branch;

        // Constructor for a empty node
        DecisionNode(bool res) : result(res), true_branch(nullptr), false_branch(nullptr) {}
        // Constructor for an internal node (with a condition)
        DecisionNode(function<bool(int)> cond, unique_ptr<DecisionNode> t_branch, unique_ptr<DecisionNode> f_branch) : condition(cond), true_branch(std::move(t_branch)), false_branch(std::move(f_branch)) {}
        // Evaluates the decision node given the current hand total
        bool evaluate(int handTotal) const {
            if (!condition) { // If it's a conditionless node, return the result (base case)
                return result;
            }
            if (condition(handTotal)) {
...
public:
    // Constructor
    DealerTree(const string& name = "The DealerTree"){

        auto standNode = make_unique<DecisionNode>(false);
        auto hitNode = make_unique<DecisionNode>(true);
        m_decisionTreeRoot = make_unique<DecisionNode>(
            [](int handTotal) { 
                return handTotal < 17; 
            },
            move(hitNode),
    ...
    /*
    new isHitting() function simplifies the function call, relying on the decision tree to determine
    if it's suitable to hit or stand.
    */
    ...
    as per game rules, the DealerTree only shows one card until the end of the sequence, and then
    reveals their hand.
    ...
    */
    ...
    /*
        overwritten deck operations, in the event of multiple decks, or just DealerTree shuffle,
...
public:
    ...
    */
    enum class GameGraph { BETTING, DEALING, PLAYER_TURN, DealerTree_TURN, PAYOUT, CLEANUP };
    ...
    Game() : m_currentState(GameGraph::BETTING){
        m_cardValues["2"] = 2;
        m_cardValues["3"] = 3;
        m_cardValues["4"] = 4;
        m_cardValues["5"] = 5;
        ...
        for the sake of demonstration, let's make the GameGraph transition a graph with functions to manage transition states.
        
        */
        m_stateTransitions[GameGraph::BETTING] = {GameGraph::DEALING};
        m_stateTransitions[GameGraph::DEALING] = {GameGraph::PLAYER_TURN};
        m_stateTransitions[GameGraph::PLAYER_TURN] = {GameGraph::DealerTree_TURN};
        m_stateTransitions[GameGraph::DealerTree_TURN] = {GameGraph::PAYOUT};
        m_stateTransitions[GameGraph::PAYOUT] = {GameGraph::CLEANUP};
        m_stateTransitions[GameGraph::CLEANUP] = {GameGraph::BETTING};
        m_stateTransitions[GameGraph::BETTING].insert(GameGraph::CLEANUP);
        m_stateTransitions[GameGraph::DEALING].insert(GameGraph::CLEANUP);
        m_stateTransitions[GameGraph::PLAYER_TURN].insert(GameGraph::CLEANUP);
        m_stateTransitions[GameGraph::DealerTree_TURN].insert(GameGraph::CLEANUP);
        m_stateTransitions[GameGraph::PAYOUT].insert(GameGraph::CLEANUP);
    }
    ...
    */
    ...
    void play(){
        ...
        m_DealerTree.shuffleDeck();
        logAction("DealerTree shuffles the deck.");

        bool contPlay = true;
        while(contPlay){
            setState(GameGraph::BETTING);
            placeBets();

            setState(GameGraph::DEALING);
            deal();

            setState(GameGraph::PLAYER_TURN);
            playerTurns();

            setState(GameGraph::DealerTree_TURN);
            DealerTreeTurn();

            setState(GameGraph::PAYOUT);
            payouts();

            setState(GameGraph::CLEANUP);
            cleanup();
            findMinMaxMoney();
    ...
    void deal(){
        ...
        */
        ...
        for(auto& p : m_players){
            p->getHandRef().add(m_DealerTree.deal());
        }
        m_DealerTree.getHandRef().add(m_DealerTree.deal());
    ...
    void DealerTreeTurn(){
        ...
        m_DealerTree.showHand(true);

        if (cleanSweep){
            cout << "Wow! All players have busted, the DealerTree wins!\n";
            logAction("All players busted, DealerTree wins");
            return;
        }

        while (m_DealerTree.isHitting()){
            Card nC = m_DealerTree.deal();
            cout << "DealerTree recieves: " << nC.getRank() << " of " << nC.getSuit() << endl;
            m_DealerTree.getHandRef().add(nC);
            cout << "DealerTree has " << m_DealerTree.getHand().getTotal() << ".\n";
        }

        if(m_DealerTree.isBusted()){
            cout << "DealerTree busts with " << m_DealerTree.getHand().getTotal() << "!\n";
            logAction("DealerTree busted with " + to_string(m_DealerTree.getHand().getTotal()));
        }
        else{
            cout << "DealerTree stands with " << m_DealerTree.getHand().getTotal() << "!\n";
            logAction("DealerTree stands with " + to_string(m_DealerTree.getHand().getTotal()));
        }

    }
    /*
    
        payout function based on hand totals and if player or DealerTree has achieved a blackjack.
        basic if else ladder logic to determine how much money is either sent out, lost, or pushed
        back to the players and DealerTree in the event of a tie.
    
    */
    void payouts(){
        cout << "\n===== RESULTS =====\n";

        int dT = m_DealerTree.getHand().getTotal();
        bool dB = m_DealerTree.isBusted();
        bool dBJ = m_DealerTree.isBlackjack();
        ...
        for (auto& p : m_players){
            ...
            if (pB){
                ...
                cout << "Won $" << p->getBet() << " (DealerTree busted).\n";
                logAction(name + " won $" + to_string(p->getBet()) + " (DealerTree busted)");
            }
            else if(pBJ && !dBJ){
                ...
                p->lose();
                m_stats.recordLoss(name);
                cout << "Lost $" << p->getBet() << " the DealerTree's blackjack. \n";
                logAction(name + "lost $ " + to_string(p->getBet()) + " to DealerTree's blackjack");
            }
            else if (pT > dT){
                p->win();
                m_stats.recordWin(name);
                cout << "Won $ " << p->getBet() << " with " << pT << " over DealerTree's " << dT << ".\n";
                logAction(name + "won $" + to_string(p->getBet()));
            }
            else if (pT < dT){
                p->lose();
                m_stats.recordLoss(name);
                cout << "Lost $" << p->getBet() << " with " << pT << " under DealerTree's " << dT << ".\n";
                logAction(name + " lost $" + to_string(p->getBet())); 
            }
            else{
                p->push();
                cout << "Push. Bet of $" << p->getBet() << " returned.\n";
                logAction(name + " pushed with DealerTree at " + to_string(pT));
            }
    ...
    */
    ...
    string stateToString(GameGraph state) const{
        switch (state){
            case GameGraph::BETTING: return "BETTING";
            case GameGraph::DEALING: return "DEALING";
            case GameGraph::PLAYER_TURN: return "PLAYER TURN";
            case GameGraph::PAYOUT: return "PAYOUT";
            case GameGraph::CLEANUP: return "CLEANUP";
    ...
    bool validateTransition(GameGraph from, GameGraph to) const{
        ...
        //checks if the current transition called is a valid transition.
        auto it = m_stateTransitions.find(from);
    ...
    void setState(GameGraph state){
        try{
            if (m_currentState == state || validateTransition(m_currentState, state)){
                logAction("Transitioning from " + stateToString(m_currentState) + " to " + stateToString(state)); 
    ...
    void showWelcomeScreen() const{
        ...
        cout << "\n=====Your goal as the player is to try and beat the DealerTree by getting as close to 21 without going over!\n";
        cout << "\n=====Each player starts with two cards. The DealerTree's first card is hidden.\n";
        cout << "\n=====Each player also must decide to hit (take another card), or stand (stop taking cards).\n";
        cout << "\n=====The DealerTree will play against you! With some mental math and a little bit of luck, you could get rich!\n";
